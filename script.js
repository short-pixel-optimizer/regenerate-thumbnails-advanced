jQuery(document).ready(function($) {
    ...
    var postFilterForm = $('#posts-filter');
    var basT = $('#bulk-action-selector-top');
    var basB = $('#bulk-action-selector-bottom');

    var basTVal = basT.val();
    var basBVal = basB.val();
    var arrIDs = [];

    if(postFilterForm[0] && basT[0] && basB[0]){

      //if apply button is pressed and dropdown option is regenerate thumbnails, don't submit
      postFilterForm.submit(function(event){
        basTVal = basT.val();
        basBVal = basB.val();

        if(basTVal=='rtaMR' || basBVal=='rtaMR'){
          arrIDs = getIDs(postFilterForm);
          console.log(arrIDs);
...
//get the id's from the media upload page of the selected checkboxes
    function getIDs(postFilterForm){
      var imgArrID = [];
      var checkboxes = postFilterForm.find('input[type="checkbox"]:checked');
      if(checkboxes.length == 0){
        console.log('nothing selected. please check the checkbox of a image');
      }else{
        var i = 0;
        ...
        while(checkboxes[i]){
          ...
          ccID = currentCheckbox.attr('id');
          //exclude the general selects and include the id selects
          if(ccID.indexOf('cb-select-')!== -1 && ccID.indexOf('select-all') === -1){
            //
            //  Where all the magic happens
            //
            //get the id number of the image

            ccID = ccID.replace('cb-select-','');
